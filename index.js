  ...
  try {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      try {
        ...
        const click = await page.$$(".info-sign");
        if (click.length == 2) {
          await click[1].click();
        ...
        const localDateTimeString = localDateTime
          .toISOString()
          .slice(0, 19)
          .replace("T", " ");
        ...
        if (existingRecord) {
          ...
          if (existingDateTime.isBefore(localDateTime)) {
            ...
            request.input("MoveStart", sql.VarChar, elements[1]);
            request.input("MoveFinishExpected", sql.VarChar, elements[0]);
